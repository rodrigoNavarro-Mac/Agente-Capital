/**
 * Reintentar handover para una conversación calificada con Lead Zoho "pending".
...
import { retryHandover } from '@/lib/modules/whatsapp/conversation-flows';
...
export async function POST(request: NextRequest): Promise<NextResponse> {
    try {
        ...
        const result = await retryHandover(user_phone, development);

        if (!result.success) {
            return NextResponse.json(
                { success: false, error: result.error || 'Error al reintentar handover' },
                { status: 400 }
            );
        }
        ...
        logger.error('Error en retry-handover', error, {}, 'whatsapp-api');
        return NextResponse.json(
            { success: false, error: 'Error interno al reintentar handover' },
            { status: 500 }
        );
    }
}
