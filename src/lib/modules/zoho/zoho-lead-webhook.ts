...
export async function handleZohoLeadCreatedWebhook(
  ...
  try {
    ...
    const sent = await sendTemplateMessage(phoneNumberId, userPhone.replace(/^\+/, ''), template.name, template.language, [firstName], template.bodyParameterNames, template.headerImageUrl);
    if (!sent) {
